/*This is a custom pci device driver written by mmssl
...
// This macro provides the instances type cast functions for QOM type.
DECLARE_INSTANCE_CHECKER(PciechodevState, PCIECHODEV, TYPE_PCI_CUSTOM_DEVICE)

// Struct defining the state
// of the custom pcidevice
...
static void pciechodev_bar0_mmio_write(void *opaque, hwaddr addr, uint64_t val,
  ...
  printf("PCIECHODEV: BAR0 pciechodev_mmio_write() addr %lx val %lx size %x \n",
         addr, val, size);
  PciechodevState *pciechodev = opaque;
  if (addr >= 64)
	  return;
...
static uint64_t pciechodev_bar1_mmio_read(void *opaque, hwaddr addr,
                                          unsigned size) {
...
static void pciechodev_bar1_mmio_write(void *opaque, hwaddr addr, uint64_t val,
                                       unsigned size) {
  printf("PCIECHODEV: pciechodev_bar0_mmio_read () addr %lx val %lx size %x \n",
         addr, val, size);
  PciechodevState *pciechodev = opaque;
  if (size == 1) {
...
// implementation of realize function
...
static void pci_pciechodev_realize(PCIDevice *pdev, Error **errp) {
  ...
  memory_region_init_io(&pciechodev->mmio_bar0, OBJECT(pciechodev),
                        &pciechodev_bar0_mmio_ops, pciechodev,
                        "pciechodev-mmio", 16);
  // registering pdev and all the above configuration
  // actually filling a PCI-IO region with this configuration
  pci_register_bar(pdev, 0, PCI_BASE_ADDRESS_SPACE_MEMORY,
                   &pciechodev->mmio_bar0);
  /* BAR 1 */
  memory_region_init_io(&pciechodev->mmio_bar1, OBJECT(pciechodev),
                        &pciechodev_bar1_mmio_ops, pciechodev, "pciechodev",
                        4096);
  pci_register_bar(pdev, 1, PCI_BASE_ADDRESS_SPACE_MEMORY,
                   &pciechodev->mmio_bar1);
}

// uninitializing function perform
static void pci_pciechodev_uninit(PCIDevice *pdev) { return; }

// initialization of the device
static void pciechodev_instance_init(Object *obj) { return; }

static void pciechodev_class_init(ObjectClass *class, void *data) {
  DeviceClass *dc = DEVICE_CLASS(class);
  PCIDeviceClass *k = PCI_DEVICE_CLASS(class);

  // definition of realize function
  k->realize = pci_pciechodev_realize;
  // definition of uninit function
  k->exit = pci_pciechodev_uninit;
  k->vendor_id = PCI_VENDOR_ID_QEMU;
  k->device_id = 0xbeef; // our device id beef (hex)
  k->revision = 0x10;
  k->class_id = PCI_CLASS_OTHERS;

  /**
   * set_bit - Set bit in a memory
   * @nr: bit to set
